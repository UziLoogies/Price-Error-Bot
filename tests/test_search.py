...
class TestSearchQueryBuilder:
    ...
    def test_tokenize_query(self):
        ...
        # Field-specific search (simple values)
        tokens = self.builder.tokenize_query("store:amazon category:electronics")
        assert tokens["field_filters"] == {"store": "amazon", "category": "electronics"}
...
class TestSearchService:
    ...
    async def test_search_products_basic(self):
        ...
        result = await self.service.search_products(
            db=mock_db, query_text="iphone", store=["amazon_us"], limit=20
        )
