...
from src.db.models import Base, Product, Alert, Rule, PriceHistory
...
class TestSearchPerformance:
    ...
    async def test_search_response_time(
        self, client: TestClient, sample_products, sample_price_history
    ):
        ...
        start_time = time.time()
        response = client.get(
            "/api/search/products?q=phone&stores=amazon_us&price_min=300&price_max=1200&sort_by=relevance"
        )
        response_time = time.time() - start_time
    ...
    async def test_price_range_accuracy(self, client: TestClient, sample_products):
        ...
        for product in products:
            ...
            if price:
                assert (
                    300 <= price <= 800
                ), f"Product {product['sku']} price {price} not in range 300-800"
...
@pytest.mark.asyncio
async def test_search_end_to_end_flow(
    client: TestClient, sample_products, sample_price_history, sample_alerts
):
