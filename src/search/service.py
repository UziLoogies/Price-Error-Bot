...
from src.db.models import (
    Product, Alert, StoreCategory, ScanJob, 
    SearchQuery, SearchSuggestion, PriceHistory
)
...
class SearchService:
    ...
    async def search_products(
        ...
        **filters
        ...
        # Generate cache key
        cache_key = f"search:products:{self.query_builder.calculate_query_hash(q=query_text, **filters)}"
        ...
        try:
                ...
                'products': [
                    {
                        ...
                        'highlights': self._generate_highlights(p, query_text) if query_text else {}
                    }
            ...
            return {
                'products': [],
                'pagination': {'has_next': False, 'has_previous': False, 'limit': 0, 'total_count': 0},
                'facets': {},
                'query_info': {
                    'query': query_text or '',
    ...
    async def search_alerts(
        ...
        **filters
        ...
        cache_key = f"search:alerts:{self.query_builder.calculate_query_hash(q=query_text, **filters)}"
        cached_result = await self._get_cached_result(cache_key)
        if cached_result:
            return cached_result
                ...
                'alerts': [
                    {
                        ...
                        'discount_percent': self._calculate_discount_percent(a.triggered_price, a.previous_price),
                        'discord_message_id': a.discord_message_id,
                        'sent_at': a.sent_at.isoformat(),
                        'highlights': self._generate_highlights(a, query_text) if query_text else {}
                    }
                    for a in alerts
                ],
                'pagination': {
    ...
    async def _get_product_suggestions(
        ...
        if not field or field == 'title':
            ...
            title_query = select(Product.title, func.count().label('count')).where(
                func.similarity(Product.title, query) > 0.3
            ).group_by(Product.title).order_by(desc('count')).limit(limit)
        ...
        if not field or field == 'sku':
            # SKU suggestions
            sku_query = select(Product.sku).where(
    ...
    async def _get_category_suggestions(
        ...
        name_query = select(StoreCategory.category_name, func.count().label('count')).where(
            func.similarity(StoreCategory.category_name, query) > 0.3
        ).group_by(StoreCategory.category_name).order_by(desc('count')).limit(limit)
    ...
    async def _get_product_facets(
        ...
        try:
            ...
            facets['stores'] = [
                {
                    'name': store,
                    'count': count,
                    'display_name': self._get_store_display_name(store)
                }
            ...
            price_stats_query = select(
                func.min(Product.msrp).label('min_price'),
                func.max(Product.msrp).label('max_price')
    ...
    async def _get_recent_searches(
        ...
        query = select(SearchQuery.query_text, SearchQuery.created_at).where(
            SearchQuery.entity_type == entity_type
        ).order_by(desc(SearchQuery.created_at)).limit(limit)
    ...
    async def _get_popular_searches(
        ...
        ).group_by(SearchQuery.query_text).order_by(desc('count')).limit(limit)
        
        result = await db.execute(query)
        return [
            {'query': query_text, 'count': count}
            for query_text, count in result.all()
        ]
