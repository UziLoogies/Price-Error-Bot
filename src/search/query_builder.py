...
from src.db.models import Product, Alert, StoreCategory, ScanJob, PriceHistory
...
class SearchQueryBuilder:
    ...
    def tokenize_query(self, query: str) -> Dict[str, Any]:
        ...
        if total_terms > self.MAX_TERMS:
            self.logger.warning(f"Too many search terms: {total_terms}, limiting to {self.MAX_TERMS}")
    ...
    def build_product_search_query(
                ...
                if tsquery:
                    ...
                    if len(normalized) >= self.FUZZY_MIN_LENGTH:
                        ...
                        search_conditions.append(
                            or_(title_similarity > 0.3, sku_similarity > 0.3)
                        )
        ...
        if price_min is not None:
            price_condition = or_(
                Product.msrp >= price_min,
                Product.baseline_price >= price_min
            )
        ...
        if price_max is not None:
            price_condition = or_(
                Product.msrp <= price_max,
                Product.baseline_price <= price_max
            )
            search_conditions.append(price_condition)
            metadata['filter_count'] += 1
        ...
        if in_stock is not None:
            ...
            if in_stock:
                search_conditions.append(latest_price_subq.c.availability.in_(["In Stock", "Limited Stock"]))
            else:
